...
// ─────────────────────────────────────────────────────────────
// Fondo + glifos decorativos del tema (la semilla). Mismo patrón que la
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 22, maxWidth: 520 }}>
          <div>
            <div className="ed-label" style={{ color: "#4fd8ff", marginBottom: 8 }}>
              EDINUN · La semilla {/* ← PERSONALIZAR: tema del juego */}
            </div>
...
function CharacterScreen({ app, setApp, go }) {
  // Predeterminado de este juego: Tomi (geólogo). El shell arranca con
  // character "astronauta", así que tratamos ese valor inicial como "sin elegir"
  // y mostramos a Tomi; si el niño ya eligió otro guía, se respeta.
  const [sel, setSel] = useState(
    app.character && app.character !== "astronauta" ? app.character : "geologo"
  );
  ...
  function choose() {
    ...
    setApp((s) => ({
      ...s,
      character: sel,
      currentCategory: "la-semilla",
      currentCatLabel: "La semilla",
    }));
    go("game");
  }
