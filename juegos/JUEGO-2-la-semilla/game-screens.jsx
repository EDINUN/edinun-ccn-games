...
// barajadas de la bandeja para no volver a salir igual al recargar.
const RECENT_KEY = "edinun_ccn_semilla_bandeja_v1";
function getRecentTrays() {
  try { const r = JSON.parse(localStorage.getItem(RECENT_KEY) || "[]"); return Array.isArray(r) ? r : []; }
  catch (e) { return []; }
}
function rememberTray(order) {
  const key = order.join(",");
  const prev = getRecentTrays().filter((k) => k !== key);
  const next = [key].concat(prev).slice(0, 3); // recuerda las últimas 3 barajadas
  try { localStorage.setItem(RECENT_KEY, JSON.stringify(next)); } catch (e) {}
}

// Orden inicial de la bandeja: barajado, nunca ya resuelto y DISTINTO a las
// últimas veces (para que no salga igual a cada niño / cada recarga).
function initialTrayOrder() {
  const recent = new Set(getRecentTrays());
  let a = shuffle([0, 1, 2, 3]);
  let guard = 0;
  while ((a.join(",") === "0,1,2,3" || recent.has(a.join(","))) && guard++ < 40) {
    a = shuffle([0, 1, 2, 3]);
  }
  return a;
}
...
function GameScreen({ app, setApp, go }) {
  ...
  // Recuerda la barajada de esta partida → la próxima carga sale distinta.
  useEffectG(() => { rememberTray(trayOrder); }, []);
